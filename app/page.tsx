...
export default function Home() {
  ...
  return <main className="shell"><section className="workspace">
    ...
    {view === "cluster" ? <>
      ...
      <section className="definition-notes"><h2>Notes · Current Definitions</h2><p className="definition-intro">以下是系统目前实际使用的分类规则。请按你的交易定义检查；下一步会以你确认的规则修改扫描算法。</p><div className="definition-grid"><article><b>Setup</b><p>原始状态为 <code>anticipation</code>。价格在 20 日与 50 日均线上方、接近近期 pivot / 高点，并保持正向的三个月动量；属于等待突破，尚未计为 breakout。</p></article><article><b>Spring</b><p>先跌破近 30 日支撑约 1%，随后收回支撑；之后 1–5 日内出现放量 momentum burst，并且当前价格仍高于支撑及 spring 当日收盘。</p></article><article><b>Breakout</b><p>目前不是独立原始标签：最新的 <code>spring</code> burst / breakout 发生在约 1 个交易日内时，归入 Breakout。Cluster Score 的“今日突破”使用此数量。</p></article><article><b>Pullback / Holding</b><p>包括 <code>bpr</code>，以及较早发生但仍有效的 spring。BPR 要求先放量突破，再回踩 21EMA 附近，并仍守在突破 pivot 上方、没有过度延伸。</p></article><article><b>Fakeout</b><p>突破后收回 pivot 下方；或最近 5 日一度高于之前 50 日高点约 1%，但最新收盘重新跌回该高点下方，同时仍在 50 日均线上方。</p></article><article><b>Watch</b><p>股票仍在强势主题池内，但暂时没有命中 Setup、Spring、BPR 或 Fakeout。它不会计入四个主要状态数量。</p></article></div><p className="method-note">Cluster Score 用于集中注意力，不是买入信号。若数据源暂未提供 RS、排名历史、均线或 RVOL，页面会使用当前动量、周表现及趋势标签作兼容估算。</p></section>
