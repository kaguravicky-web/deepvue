...
export default function Home() {
  ...
  return <main className="shell"><section className="workspace">
    ...
    {view === "cluster" ? <>
      ...
      <div className="cluster-toolbar"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索群组、主题或股票" /><label>聚合<select value={groupBy} onChange={(event) => setGroupBy(event.target.value as GroupBy)}><option value="group">Group</option><option value="industry">Industry</option><option value="theme">Theme</option></select></label><label>排序<select value={sortBy} onChange={(event) => setSortBy(event.target.value as SortBy)}><option value="score">Cluster Score</option><option value="breakout">Breakout</option><option value="setup">Setup</option><option value="holding">Holding</option><option value="fakeout">Fakeout</option><option value="rank">Rank</option></select></label><label>最低分<select value={minScore} onChange={(event) => setMinScore(Number(event.target.value))}>{[0,4,7,9].map((value) => <option key={value} value={value}>{value}+</option>)}</select></label></div>
