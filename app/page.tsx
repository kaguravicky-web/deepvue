...
type Candidate = {
  ...
  rs?: number; relativeVolume?: number; volumeRatio?: number; range15Pct?: number; closePosition?: number; pivot?: number;
};
...
function TickerList({ items, empty = "—", withTotal = false }: { items: EnrichedCandidate[]; empty?: string; withTotal?: boolean }) {
  ...
  return <div className="ticker-list">{items.slice(0, 5).map((item) => <a key={item.ticker} href={tradingView(item.ticker)} target="_blank" rel="noreferrer" title={`${item.name} · RS ${item.computedRs}`}>{item.ticker}{(item.alert?.includes("🔥") || item.ticker === "OOMA") && <span aria-label="hot">🔥</span>}</a>)}{withTotal && <span className="ticker-total">({items.length})</span>}</div>;
}
